...
from Matrix import *
import Matrix_non_square
import sys

class Misty(object):
    def __init__(self, matNameFL, matrixFL,
                       matNameFL_1, matrixFL_1,
                       matNameFO, matrixFO,
        ...
        self._matrixNameFL_1 = matNameFL_1
        self._matrixFL_1 = matrixFL_1
    ...
    def _FL(self, in_vars, out_vars, r, p, flg=0):
        if flg == 0:
            m = Matrix( self._matrixNameFL, self._matrixFL, in_vars, out_vars, r, p)
        else:
            m = Matrix( self._matrixNameFL_1, self._matrixFL_1, in_vars, out_vars, r, p)
    ...
    def _gen_init_constrs(self):
        ...
        for i in range(self._dim):
            if i in self._outVec:
                self._addConstr( 'ASSERT %s = 0bin1;' % self._vars['MISTY_KX'][self._round][i] )
            else:
